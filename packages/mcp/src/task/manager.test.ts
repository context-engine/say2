...
describe("TaskManager", () => {
	...
	describe("registerTask", () => {
		...
		it("overwrites existing task with same ID", () => {
			...
			manager.registerTask("task-dup", "session-1", {
	...
	describe("pollUntilComplete", () => {
		it("returns immediately on completed status", async () => {
			...
			const result = await manager.pollUntilComplete("task-done", async () => {
				pollCount++;
				return {
					taskId: "task-done",
					status: "completed",
		...
		it("returns immediately on failed status", async () => {
			...
			const result = await manager.pollUntilComplete("task-fail", async () => ({
				taskId: "task-fail",
				status: "failed",
		...
		it("respects task pollInterval when provided", async () => {
			...
			await manager.pollUntilComplete("task-interval", async () => {
				pollCount++;
				return {
		...
		it("throws error after max poll attempts", async () => {
			...
			await expect(
				manager.pollUntilComplete("task-timeout", async () => ({
					taskId: "task-timeout",
		...
		it("updates cache on each poll", async () => {
			manager.registerTask("task-cache", "session-1", {
				statusMessage: "Initial",
			});
			let pollCount = 0;

			await manager.pollUntilComplete("task-cache", async () => {
				pollCount++;
				return {
					taskId: "task-cache",
					status: pollCount >= 2 ? "completed" : "working",
	...
	describe("handleStatusNotification", () => {
		it("updates cached task", () => {
			manager.registerTask("task-notify", "session-1", {
