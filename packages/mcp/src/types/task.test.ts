...
describe("Edge Cases", () => {
	...
	it("Task with all terminal statuses validates", () => {
		...
		expect(TaskSchema.parse({ ...baseTask, status: "completed" }).status).toBe(
			"completed",
		);
		expect(TaskSchema.parse({ ...baseTask, status: "failed" }).status).toBe(
			"failed",
		);
		expect(TaskSchema.parse({ ...baseTask, status: "cancelled" }).status).toBe(
			"cancelled",
		);
	});
