...
describe("STDIO Transport Integration", () => {
	...
	test("fails when command does not exist", async () => {
		...
		try {
			...
			// biome-ignore lint/suspicious/noExplicitAny: needed for test error capture
		} catch (e: any) {
			expect(e).toBeDefined();
			// ENOENT or similar depending on platform, but definitely an error
			expect(e.message ?? e.toString()).toMatch(/spawn|enoent|found/i);
		}
	});
