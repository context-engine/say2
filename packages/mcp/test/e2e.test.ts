...
describe("MCP E2E Integration", () => {
	...
	describe("session lifecycle", () => {
		test("create session → connect → active → close", async () => {
			...
			try {
				...
				expect(
					...
					].includes(updatedSession?.state as typeof SessionState.ACTIVE),
				).toBe(true);
				...
				expect(
					[SessionState.CLOSED, SessionState.ERROR].includes(
						closedSession?.state as typeof SessionState.CLOSED,
					),
				).toBe(true);
			} catch (error) {
		...
		test("session state transitions in correct order", async () => {
			...
			const activateResult = sessionManager.activate(
				session.id,
				{},
				{},
				"2024-11-05",
	...
	describe("message flow", () => {
		test("messages flow through pipeline and are stored", async () => {
			...
			let _pipelineProcessCount = 0;
			pipeline.use(async (_ctx, next) => {
				_pipelineProcessCount++;
				await next();
			});
