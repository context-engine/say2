...
describe("Resources Templates List", () => {
	test("returns resource templates when configured", () => {
		...
		expect(response.result.resourceTemplates[0].uriTemplate).toBe(
			"file:///{path}",
		);
		expect(response.result.resourceTemplates[0].name).toBe("File Template");
		expect(response.result.resourceTemplates[1].uriTemplate).toBe(
			"db://{table}/{id}",
		);
	});
	...
	test("templates include description when provided", () => {
		...
		expect(response.result.resourceTemplates[0].description).toBe(
			"Call API endpoints",
		);
	});
});
...
describe("Prompts List", () => {
	...
	test("prompt includes name and description", () => {
		const config = {
			...
			version: "1.0.0",
			capabilities: { prompts: true },
			prompts: [{ name: "code-review", description: "Review code for issues" }],
		};
