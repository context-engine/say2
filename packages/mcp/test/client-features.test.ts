...
describe("Client Features Integration Tests", () => {
    ...
    beforeEach(() => {
        ...
        pipeline.use(createStateMachineMiddleware(sessionManager));
    ...
    async function setupConnectedClient(serverConfig: any) {
        // 1. Create session
        const session = sessionManager.create({ name: "test", transport: "stdio", command: "node" });

        // 2. Setup transport stack
        const mockTransport = createMockServerTransport(serverConfig);
        const loggingTransport = new LoggingTransport(mockTransport, session, pipeline);
    ...
    test("Resource Templates: lists templates via Manager", async () => {
        const config = {
            ...
            resourceTemplates: [
                ...
                { uriTemplate: "db://{id}", name: "DB", description: "Database access" }
            ]
        };
    ...
    test("Transport Events: LoggingTransport emits events", async () => {
        ...
        // Verify Close
        let closeEmit = false;
        loggingTransport.onclose = () => { closeEmit = true };
