...
const mockClientListTools = mock(async () => ({ tools: [], nextCursor: undefined }));
...
describe("McpClientManager", () => {
    ...
    describe("connect", () => {
        test("throws error if session not found", async () => {
            await expect(clientManager.connect("non-existent-session")).rejects.toThrow(
                /not found/i,
            );
        });
        ...
        test("transitions session state to CONNECTING", async () => {
            ...
            expect([
                ...
                SessionState.ACTIVE,
                SessionState.ERROR,
            ].includes(updatedSession!.state as typeof SessionState.CONNECTING)).toBe(true);
        });
